...
\\\\
Figure~\ref{fig:oneshot-prompt} illustrates the prompt structure provided to \texttt{FLAN-T5}. The prompt takes a query, a known passage (referred to as \texttt{rel\_doc\_text}), and a passage to judge (referred to as \texttt{unk\_doc\_text}) as inputs for pairwise inference. \colorbox{red}{The} \colorbox{red}{model then predicts the probability values by computing the softmax over the} \colorbox{red}{logits of two tokens, which are subsequently used as the relevance score}. If the passage to judge is highly relevant to the query, the model's expected output should be a score close to 1.0. Conversely, if the passage to judge is not relevant at all for the query, the model should produce a score close to 0.0.
\\\\\\\\
