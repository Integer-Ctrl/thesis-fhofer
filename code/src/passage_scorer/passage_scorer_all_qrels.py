...
PASSAGE_DATASET_SCORE_ALL_QRELS_PATH = os.path.join(DATA_PATH, config['PASSAGE_DATASET_SCORE_ALL_QRELS_PATH'])
...
for index, row in tqdm(qrels.iterrows(), desc='Caching qrels', unit='qrel'):
    ...
    if row['qid'] not in qrels_cache:
        qrels_cache[row['qid']] = qrels.loc[
            (qrels['qid'] == row['qid'])  # All relevant entries for the query ID
...
with gzip.open(PASSAGE_DATASET_SCORE_ALL_QRELS_PATH, 'wt', encoding='UTF-8') as file:
