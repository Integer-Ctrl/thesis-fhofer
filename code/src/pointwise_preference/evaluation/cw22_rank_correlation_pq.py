...
def ray_wrapper(JOB_ID, NUM_JOBS, qrels_cache):
    ...
    for backbone in BACKBONES:
        ...
        POINTWISE_PREFERENCES_PATTERN = os.path.join(TARGET_PATH, MONOPROMPT_PATH, backbone, 'eval_candidates.jsonl.gz')  # glob pattern
    ...
    for qid in QUERIES:
        ...
        with open(annotated_qid_path, 'r') as file:
            for line in file:
                ...
                if len(qrel['label']) != 1:
                    print(f"Query {qid} has not one label for document {doc_id}")
                    continue
