...
CROSS_VALIDATION_SCORES_PATH = os.path.join(OLD_PATH, config['CROSS_VALIDATION_PATH'])


# 1. Load correlation scores per evaluation method and retriever
#    KEY1: eval_method___retriever___metric
#    KEY2: aggregation_method___transformation_method
...
for pt_retriever in PT_RETRIEVERS:
    for eva_method in EVALUATION_METHODS:
        for metric in METRICS:
            ...
            key1 = get_key([eva_method, pt_retriever, metric])
            total_score = 0
...
with gzip.open(CROSS_VALIDATION_SCORES_PATH, 'wt', encoding='UTF-8') as file:
    for key, score in cross_validation_scores.items():
        file.write(json.dumps({'eval_method___retriever___metric': key, 'score': score}) + '\n')
