...
PASSAGE_SCORES_PATH = '../data/argsme/passage-dataset/tfidf-scores.jsonl.gz'
        ...
        if row['label'] > 0:  # only relevant docs
            ...
            for passage in passages:
                if passage['docno'].startswith(row['docno']):
                    run = TrecRun()
                    run.run_data = tfidf.search(pt_tokenize(passage['text'])).loc[
                    ...
                    run.run_data['system'] = 'tfidf'  # dummy value to get ndcg score
